...
def move_kvadratce(state, index, dir):
    kvadratce = list(state[index])
    # print("Moving: ", kvadratce)
    kvadratce[0] += dir[0]
    kvadratce[1] += dir[1]
    kvadratce = tuple(kvadratce)
    new_state = []
    for i in range(len(state)):
        if i == index:
            new_state.append(kvadratce)
        else:
...
class Squares(Problem):
    ...
    def is_kvadratce_in_pos(self, state, index):
        return state[index] == self.goal[index]
    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            if self.is_kvadratce_in_pos(state, i):
                continue
            for action, direction in zip(actions, directions):
                new_state = move_kvadratce(state, i, direction)
                if new_state != state:
                    succ[f"Pomesti kvadratche {i + 1} {action}"] = new_state
        return succ
