...
def move_gore(state, steps, walls, n):
    ghost = list(state)
    ghost[1] += steps
    new_state = tuple(ghost)
    if GhostOnSkates.check_valid(new_state, walls, n):
        return new_state
    else:
        return state


def move_desno(state, steps, walls, n):
    ghost = list(state)
    ghost[0] += steps
    new_state = tuple(ghost)
    if GhostOnSkates.check_valid(new_state, walls, n):
...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        ...
        if ghost[0] < 0 or ghost[0] >= n or ghost[1] < 0 or ghost[1] >= n:
            return False
    ...
    def successor(self, state):
        ...
        for i in range(3):
            new_state = move_gore(state, i + 1, self.walls, self.n)
            if new_state != state:
                successors[f"Gore {i + 1}"] = new_state
            new_state = move_desno(state, i + 1, self.walls, self.n)
            if new_state != state:
    ...
    def h(self, node):
        ghost = node.state
        pacman = self.goal
        return manhattan_distance(ghost, pacman)
