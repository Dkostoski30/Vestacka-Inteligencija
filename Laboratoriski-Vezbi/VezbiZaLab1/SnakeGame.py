...
class Snake_Game(Problem):
    ...
    def successor(self, state):
        ...
        if self.is_move_valid(state) and new_state[0] != snake:
            possible_moves["ProdolzhiPravo"] = new_state
            snake = new_state[0]
        new_state = self.svrtiDesno(state)
        if self.is_move_valid(new_state) and new_state[0] != snake:
    ...
    def prodolziNapred(self, state):
        ...
        if snake_head in green_apples:
            ...
            green_apples = [apple for apple in green_apples if apple not in snake]
